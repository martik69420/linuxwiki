document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadCommands() {
    ...
    if (!saved) return JSON.parse(JSON.stringify(defaultCommands));
    try {
  ...
  function renderCommands(items, query = "") {
    list.innerHTML = "";
    items.forEach(cmd => {
      const li = document.createElement("li");
      li.style.cursor = "pointer";
  ...
  function showDetails(cmd) {
    ...
    // Show edit button when a command is selected
  ...
  resetBtn.onclick = () => {
    ...
    if(!original) return;
    Object.keys(original).forEach(k => currentCommand[k] = JSON.parse(JSON.stringify(original[k])));
    saveCommands();
