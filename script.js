...
const defaultCommands = [
  { name: "ls", category: undefined, description: "Lists files and directories in the current or specified path.", syntax: "ls [options] [path]", examples: ["ls -lah","ls /var/log","ls -R"], commonErrors: ["Forgetting -h with -l","Using ls without quotes for filenames with spaces"], tips: ["Use ls --color=auto for colored output","Combine with grep: ls | grep pattern"], related: ["cd","pwd"], images: [] },
  { name: "cd", category: "Filesystem", description: "Changes the current directory.", syntax: "cd [directory]", examples: ["cd /home/user","cd ..","cd ~"], commonErrors: ["Not quoting paths with spaces"], tips: ["cd - returns to previous directory"], related: ["pwd","ls"], images: [] },
  { name: "pwd", category: "Filesystem", description: "Prints the current working directory.", syntax: "pwd", examples: ["pwd"], commonErrors: [], tips: [], related: ["cd","ls"], images: [] }
];
...
function loadCommands() {
  ...
  try { return JSON.parse(saved); } catch { return structuredClone(defaultCommands); }
}
...
function getCategory(cmd) {
  return cmd.category || "Uncategorized";
}

function renderCategories() {
  const categories = ["All", ...new Set(commands.map(getCategory))];
  ...
  categories.forEach(cat => {
    ...
    li.onclick = () => { activeCategory = cat; renderCategories(); renderFiltered(); };
...
function renderFiltered() {
  ...
  const filtered = commands.filter(cmd => {
    ...
    const matchesCategory = activeCategory === "All" || getCategory(cmd) === activeCategory;
...
function render(items, query = "") {
  ...
  items.forEach(cmd => {
    ...
    } else li.textContent = `${cmd.name} — ${cmd.description}`;
...
function showDetails(cmd) {
  ...
  detailDiv.innerHTML = `
    <div class="card" data-section="description"><h2>${cmd.name}</h2><p class="editable">${cmd.description}</p></div>
    <div class="card" data-section="syntax"><h3>Syntax</h3><pre class="editable">${cmd.syntax}</pre></div>
    <div class="card" data-section="examples"><h3>Examples</h3>${cmd.examples.map(e=>`<pre class="editable">${e}</pre>`).join("")}</div>
    <div class="card" data-section="commonErrors"><h3>Common Errors</h3><ul class="editable">${cmd.commonErrors.map(e=>`<li>${e}</li>`).join("")}</ul></div>
    <div class="card" data-section="tips"><h3>Tips</h3><ul class="editable">${cmd.tips.map(e=>`<li>${e}</li>`).join("")}</ul></div>
    <div class="card" data-section="related"><h3>Related</h3><p>${cmd.related.map(r=>`<span class="related" data-name="${r}">${r}</span>`).join(", ")}</p></div>
  `;

  detailDiv.querySelectorAll(".related").forEach(span => {
    span.style.cursor="pointer";
    span.onclick=()=>{ const relatedCmd = commands.find(c=>c.name===span.dataset.name); if(relatedCmd) showDetails(relatedCmd); };
  });
...
editBtn.onclick = ()=>{
  ...
  if(editing){
    ...
    detailDiv.querySelectorAll(".card").forEach(card=>{
      ...
      else { const el=card.querySelector(".editable"); if(el) currentCommand[section]=el.innerText.trim(); }
    });
    saveCommands();
    renderFiltered();
